      ...
      <el-col :span="8" v-for="(item, idx) in row" :key="idx" >
        <div class="grid-content bg-purple-light">
          ...
          <div class="button-list" v-show="userKind !== '-2'">
            <el-switch v-model="item.status" active-value="1" inactive-value="0" active-color="#13ce66" inactive-color="#ff4949" @change="editStatus(item)"></el-switch>
  ...
  methods:{
    ...
    queryAll() {
      this.$axios.get('/announcement/queryAll', {
        ...
        // 初始化每个公告的开关状态
        this.announcementData.forEach(item => {
          this.$set(this.switchValue, item.id, item.status === 1)
        })
      })
    },
    // 打开修改页面
